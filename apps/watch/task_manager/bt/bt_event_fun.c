...
extern int __attribute__((weak))  ui_phonebook_packet_handler(u8 type, const u8 *name, const u8 *number, const u8 *date);
/*----------------------------------------------------------------------------*/
/**@brief    蓝牙电话本获取回调函数
   @param
   @return
...
void phonebook_packet_handler(u8 type, const u8 *name, const u8 *number, const u8 *date)
{
    ...
    if (ui_phonebook_packet_handler && ui_phonebook_packet_handler(type, name, number, date)) {
        return;
    }
    ...
    /* putchar('\n'); */
    /* void phonebook_save_to_flash(u8 type, const u8 *name, const u8 *number, const u8 *date); */
    /* phonebook_save_to_flash(type, name, number, date); */
}
...
int bt_wait_connect_and_phone_connect_switch(void *p)
{
    ...
    if (bt_user_priv_var.auto_connection_counter <= 0) {
        ...
        //只回连一个设备
        if (0) {//(get_current_poweron_memory_search_index(NULL)) {
            bt_init_ok_search_index();
            return bt_wait_connect_and_phone_connect_switch(0);
        } else {
            //超时断开回连结束后，经低功耗晶振设回默认值
            extern const struct low_power_param power_param;
            extern void low_power_reset_osc_type(u8 type);
            low_power_reset_osc_type(power_param.osc_type);
...
void bt_check_enter_sniff()
{
    ...
    extern u8 file_trans_idle_query(void);
    if (!file_trans_idle_query()) {
        return;
    }
#endif
    ...
    for (i = 0; i < conn_cnt; i++) {
        ...
        if (get_role_type_by_addr(sniff_ctrl_config.sniff_addr)) {
            user_emitter_cmd_prepare(USER_CTRL_SNIFF_IN, sizeof(struct sniff_ctrl_config_t), (u8 *)&sniff_ctrl_config);
        } else {
...
void sys_auto_sniff_controle(u8 enable, u8 *addr)
{
    ...
    if (enable) {
        ...
        if (bt_user_priv_var.sniff_timer) {
            ...
            sys_timer_del(bt_user_priv_var.sniff_timer);
...
#if SOUNDCARD_ENABLE
    soundcard_bt_connect_status_event(bt->event);
...
#endif
    /* void get_phonebook_test(u8 type); */
    /* get_phonebook_test(0); */
...
#endif
    bt_close_bredr();
    printf("bt_close_bredr33333333333===========");
    return;
#endif
...
void bt_status_connect(struct bt_event *bt)
{
    ...
    if (bt->value) {
#if (TCFG_LUA_ENABLE)
        emitter_save_remote_name(mac_addr, bt_remote_name);
#else /* #if (TCFG_LUA_ENABLE) */
        extern int page_list_add(u8 * mac, u8 * name); //添加收藏
        page_list_add(mac_addr, bt_remote_name);
...
#endif
    g_printf("bt->value:%d", bt->value);
#if AUDIO_OUTPUT_INCLUDE_BT
    if (bt->value) {
        bt_emitter_audio_set_mute(0);
    }
#endif /* #if AUDIO_OUTPUT_INCLUDE_BT */

    extern void bt_close_bredr_timeout_start();
...
*/
/*----------------------------------------------------------------------------*/
void bt_set_music_device_style(u8 style)
{
    __this->music_dev = style;
}

u8 bt_get_music_device_style()
{
    return __this->music_dev;
}

static int bt_init_music_device_style(void)
{
#if TCFG_USER_EMITTER_ENABLE
    __this->music_dev = SET_MUSIC_IN_WATCH;
#else
    __this->music_dev = SET_MUSIC_IN_PHONE;
#endif
    return 0;
}
platform_initcall(bt_init_music_device_style);
...
#if (TCFG_AUTO_STOP_PAGE_SCAN_TIME && TCFG_BD_NUM == 2)
    if ((get_total_connect_dev() == 1) && (bt->value != ERROR_CODE_CONNECTION_TIMEOUT)) {   //当前有一台连接上了,并且连接的是sink设备
        if (app_var.auto_stop_page_scan_timer == 0) {
            app_var.auto_stop_page_scan_timer = sys_timeout_add(NULL, bt_close_page_scan, (TCFG_AUTO_STOP_PAGE_SCAN_TIME * 1000)); //2
        }
    } else {
